...
pub fn build_and<'ctx>(
    ...
    let right_bb = codegen.context.append_basic_block(function, "and_right");
    ...
    let phi = codegen
    .builder
    .build_phi(codegen.context.bool_type(), "and_result")
    .unwrap();
    phi.add_incoming(&[
    (&codegen.context.bool_type().const_int(0, false), merge_bb),
    (&right_bool, right_bb),
    ]);
    Some(
    codegen.builder
