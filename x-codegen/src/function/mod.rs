use inkwell::values::{FloatValue, FunctionValue, BasicMetadataValueEnum};
    ...
    pub fn gen_function_call(&self, name: &str, args: &[Expr]) -> Result<FloatValue<'ctx>, String> {
        match (name, args.first()) {
            ...
            _ => {
                ...
                let compiled_args: Vec<BasicMetadataValueEnum<'ctx>> = args.iter()
    ...
    pub fn compile_function(
        ...
        for (i, param_name) in params.iter().enumerate() {
            ...
            let ptr = if param_name == "name" {
                ...
                self.builder.build_alloca(self.context.f64_type(), param_name)
                    .map_err(|e| e.to_string())?
            };
            self.builder
